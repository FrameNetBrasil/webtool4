...
class CalculateTTestTempoMedioAnotacao extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:calculate-t-test-tempo-medio-anotacao';
     ...
     * @var string
     */
    protected $description = 'Calculate paired t-test for Tempo médio de anotação data';
    ...
    {
        $inputFile = base_path('app/Console/Commands/Reporter_Brasil/test/Dados para avaliação paper LREC 2026 Repórter Brasil - Tempo médio de anotação.csv');
        $outputFile = base_path('app/Console/Commands/Reporter_Brasil/test/Dados para avaliação paper LREC 2026 Repórter Brasil - Tempo médio de anotação - with t-test.csv');
        ...
        // Extract column data (columns D, E are indices 3, 4 - Human only, LOME + Human)
        $columnD = [];
        $columnE = [];

        foreach ($dataRows as $row) {
            $columnD[] = $this->parseNumber($row[3]);
            $columnE[] = $this->parseNumber($row[4]);
        }

        // Calculate paired t-test for annotation time (D vs E)
        $tTestDE = $this->calculatePairedTTest($columnD, $columnE);

        // Display results
        $this->info('Paired T-Test Results:');
        $this->newLine();

        $this->info('=== Annotation Time comparisons ===');
        $this->displayTTestResults('D vs E (Human only vs LOME + Human)', $tTestDE);

        // Add results to CSV
        $header[] = 't-statistic (D-E)';
        $header[] = 'p-value (D-E)';
        $header[] = 'df (D-E)';
        ...
        $summaryRow[] = number_format($tTestDE['t'], 6);
        $summaryRow[] = number_format($tTestDE['p'], 6);
        $summaryRow[] = $tTestDE['df'];
    ...
    private function displayTTestResults(string $label, array $results): void
    {
        ...
        $this->line("  Mean diff:   " . number_format($results['mean_diff'], 6) . " seconds");
