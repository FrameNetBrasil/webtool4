...
class Lexeme extends Repository {
    ...
    public static function listByFilter($filter)
    {
        ...
        $criteria = self::getCriteria()
            ->select(['idLexeme','name','pos.POS', 'idEntity','lexemeEntries.lexemeOrder','lexemeEntries.headWord'])
            ->distinct()
            ->orderBy('lexemeEntries.lexemeOrder,name');
        return self::filter([
            ['idLanguage','=',$idLanguage],
            ['idLexeme','=',$filter?->idLexeme ?? null],
            ['name','startswith',$filter?->name ?? null],
            ['name','startswith',$filter?->lexeme ?? null],
            ['lexemeEntries.idLemma','=',$filter?->idLemma ?? null],
        ], $criteria);
    }
    /*
