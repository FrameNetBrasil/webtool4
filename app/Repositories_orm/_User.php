...
class _User //extends Repository
{
...
//    }
    ...
    public static function listToGrid(SearchData $search): array
    {
        $criteria = self::getCriteria()
            ->select(['groups.idGroup', 'idUser', 'login', 'name', 'if(status = 1,"authorized","pending") as status'])
            ->orderBy('groups.idGroup')
            ->orderBy('login');
        $criteria->where('groups.name', 'startswith', $search?->group);
        $criteria->where(function (Criteria $c) use ($search) {
            $c->where('name', 'startswith', $search?->user)
                ->orWhere('email', 'startswith', $search?->user)
                ->orWhere('login', 'startswith', $search?->user);
        });
