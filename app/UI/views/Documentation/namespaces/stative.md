...
Stative frames foreground the **FORMAL ROLE**, emphasizing **conditions, properties, or relations** that hold over time without internal change or dynamism. 
These frames center on entities being in particular states, having properties, or standing in relations to other entities, rather than undergoing events or processes.
...
**Philosophical grounding**: States relate to **properties** (what something is like), **categories** (what kind of thing something is), and **relations** (how things stand to each other). 
This connects to the FORMAL quale - the essential nature or classification of entities.
...
1. **Temporariness**: Can begin and end without changing entity's identity
   - *João está cansado agora, mas não estava ontem* (is tired now, wasn't yesterday)

2. **Compatibility with location/time modification**:
   ```
   ✓ João está cansado em casa
   ✓ Maria estava feliz durante a festa
   ```

3. **Can serve as complements to perception verbs**:
   ```
   ✓ Eu vi João cansado (I saw João tired)
   ✓ Encontrei Maria feliz (I found Maria happy)
   ```

4. **Compatible with *quando* (when) clauses**:
   ```
   ✓ Quando João está cansado, ele dorme cedo
   (When João is tired, he sleeps early)
   ```

5. **Existential constructions possible**:
   ```
   ✓ Há muitas pessoas cansadas aqui
   (There are many tired people here)
   ```
...
1. **Permanence/stability**: Hold across time, rarely change
   - *João é brasileiro* (holds throughout life, essentially)

2. **Incompatibility with temporal/locational modification**:
   ```
   ✗ *João é brasileiro em casa (odd - property doesn't vary by location)
   ✗ *Maria é alta às terças (odd - property doesn't vary by day)
   ```

3. **Incompatible with perception verb complements**:
   ```
   ✗ *Eu vi João brasileiro (odd - can't perceive nationality directly)
   ? Eu vi João alto (marginal - perceive height only indirectly)
   ```

4. **Incompatible with *quando* clauses** (usually):
   ```
   ✗ *Quando João é brasileiro, ele fala português
   (odd - being Brazilian is constant, not conditional)
   ```

5. **Generic/law-like interpretations**:
   ```
...
1. **Taxonomic hierarchies**: Support subsumption reasoning
   ```
   João é humano
   Humano é mamífero
   ∴ João é mamífero
...
2. **Necessarily with *ser***: Cannot use *estar* for categories
   ```
   ✓ João é médico (João is a doctor)
   ✗ *João está médico (ungrammatical)
   ```

3. **Generic interpretation**: Often express kind-level predicates
   ```
...
1. **Inchoative/change-of-state** (become)
2. **Permanent geographic location** (be located)
...
[//]: # (### **10. Recommendations for FrameNet Brasil / DAISY**)

[//]: # ()
[//]: # (1. **Tag stative frames with subtype features**:)

[//]: # (   ```)

[//]: # (   [stative_type: individual_level|stage_level|locational|attributive|categorical])

[//]: # (   [copula: ser|estar|ficar|ter|none])

[//]: # (   [participant_structure: monadic|dyadic_asymmetric|dyadic_symmetric])

[//]: # (   [coercion_potential: inchoative|iterative|agentive|none])

[//]: # (   ```)

[//]: # ()
[//]: # (2. **Distinguish copular statives by copula**:)

[//]: # (   - SER-frames: Individual-level properties, categories, permanent states)

[//]: # (   - ESTAR-frames: Stage-level conditions, temporary locations, result states)

[//]: # (   - FICAR-frames: Either inchoative &#40;becoming&#41; OR permanent location)

[//]: # ()
[//]: # (3. **Mark coercion contexts explicitly**:)

[//]: # (   - When FICAR used → tag as Inchoative namespace &#40;not Stative&#41;)

[//]: # (   - When progressive on stative → tag as coerced eventive reading)

[//]: # (   - When *estar sendo* → tag as volitional behavioral reading)

[//]: # ()
[//]: # (4. **For DAISY parsing**: Use diagnostic tests for frame disambiguation:)

[//]: # (   - If progressive incompatible → Stative frame)

[//]: # (   - If temporal modification odd → Individual-level stative)

[//]: # (   - If copula = SER → Likely individual-level or categorical)

[//]: # (   - If copula = ESTAR → Likely stage-level or locational)

[//]: # ()
[//]: # (5. **Encode participant structure**:)

[//]: # (   - Locational: Always mark Figure and Ground roles)

[//]: # (   - Attributive: Mark entity and property/attribute)

[//]: # ()
[//]: # (6. **Cross-namespace relations**:)

[//]: # (   - Link inchoative coercions: Stative frame ↔ Inchoative frame via FICAR)

[//]: # (   - Example: *Felicidade* &#40;happiness-state&#41; ↔ *Tornar-se_feliz* &#40;become-happy&#41;)
