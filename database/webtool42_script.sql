...
CREATE TABLE IF NOT EXISTS `webtool42_db`.`sentence` (
  ...
  `text` VARCHAR(4000) CHARACTER SET 'utf8mb4' COLLATE 'utf8mb4_general_ci' NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `webtool42_db`.`dynamicobject` (
  ...
  INDEX `idx_dynamicobject_status` (`status` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `webtool42_db`.`staticobject` (
  ...
  INDEX `idx_staticobject_status` (`status` ASC) VISIBLE,
  INDEX `idx_staticobject_name` (`name` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `webtool42_db`.`lu` (
  ...
  `idUser` INT(11) NULL,
  `suggestedNewFrame` VARCHAR(45) NULL,
  ...
  INDEX `idx_lu_name` (`name` ASC) VISIBLE,
  INDEX `fk_lu_frame1_idx` (`idFrame` ASC) VISIBLE,
...
-- -----------------------------------------------------
-- Table `webtool42_db`.`lexicon_pattern`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`lexicon_pattern` (
  `idLexiconPattern` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `idLexicon` INT(10) UNSIGNED NOT NULL,
  `patternType` VARCHAR(255) NOT NULL DEFAULT 'canonical',
  `created_at` TIMESTAMP NULL DEFAULT NULL,
  `updated_at` TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (`idLexiconPattern`),
  UNIQUE INDEX `lexicon_pattern_idlexicon_patterntype_unique` (`idLexicon` ASC, `patternType` ASC) VISIBLE,
  CONSTRAINT `lexicon_pattern_idlexicon_foreign`
    FOREIGN KEY (`idLexicon`)
    REFERENCES `webtool42_db`.`lexicon` (`idLexicon`)
    ON DELETE CASCADE)
ENGINE = InnoDB
...
-- -----------------------------------------------------
-- Table `webtool42_db`.`lexicon_pattern_constraint`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`lexicon_pattern_constraint` (
  `idLexiconPatternConstraint` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `idLexiconPattern` INT(10) UNSIGNED NOT NULL,
  `constraintType` VARCHAR(255) NOT NULL,
  `constraintValue` TEXT NOT NULL,
  `created_at` TIMESTAMP NULL DEFAULT NULL,
  `updated_at` TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (`idLexiconPatternConstraint`),
  INDEX `lexicon_pattern_constraint_idlexiconpattern_constrainttype_index` (`idLexiconPattern` ASC, `constraintType` ASC) VISIBLE,
  CONSTRAINT `lexicon_pattern_constraint_idlexiconpattern_foreign`
    FOREIGN KEY (`idLexiconPattern`)
    REFERENCES `webtool42_db`.`lexicon_pattern` (`idLexiconPattern`)
    ON DELETE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_unicode_ci;
...
-- -----------------------------------------------------
-- Table `webtool42_db`.`lexicon_pattern_node`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`lexicon_pattern_node` (
  `idLexiconPatternNode` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `idLexiconPattern` INT(10) UNSIGNED NOT NULL,
  `position` INT(11) NOT NULL,
  `idLexicon` INT(10) UNSIGNED NULL DEFAULT NULL,
  `idUDPOS` INT(10) UNSIGNED NULL DEFAULT NULL,
  `isRoot` TINYINT(1) NOT NULL DEFAULT 0,
  `isRequired` TINYINT(1) NOT NULL DEFAULT 1,
  `created_at` TIMESTAMP NULL DEFAULT NULL,
  `updated_at` TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (`idLexiconPatternNode`),
  UNIQUE INDEX `lexicon_pattern_node_idlexiconpattern_position_unique` (`idLexiconPattern` ASC, `position` ASC) VISIBLE,
  INDEX `lexicon_pattern_node_idlexicon_foreign` (`idLexicon` ASC) VISIBLE,
  INDEX `lexicon_pattern_node_idudpos_foreign` (`idUDPOS` ASC) VISIBLE,
  INDEX `lexicon_pattern_node_idlexiconpattern_isroot_index` (`idLexiconPattern` ASC, `isRoot` ASC) VISIBLE,
  CONSTRAINT `lexicon_pattern_node_idlexicon_foreign`
    FOREIGN KEY (`idLexicon`)
    REFERENCES `webtool42_db`.`lexicon` (`idLexicon`)
    ON DELETE SET NULL,
  CONSTRAINT `lexicon_pattern_node_idlexiconpattern_foreign`
    FOREIGN KEY (`idLexiconPattern`)
    REFERENCES `webtool42_db`.`lexicon_pattern` (`idLexiconPattern`)
    ON DELETE CASCADE,
  CONSTRAINT `lexicon_pattern_node_idudpos_foreign`
    FOREIGN KEY (`idUDPOS`)
    REFERENCES `webtool42_db`.`udpos` (`idUDPOS`)
    ON DELETE SET NULL)
ENGINE = InnoDB
...
-- -----------------------------------------------------
-- Table `webtool42_db`.`lexicon_pattern_edge`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`lexicon_pattern_edge` (
  `idLexiconPatternEdge` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `idLexiconPattern` INT(10) UNSIGNED NOT NULL,
  `idNodeHead` INT(10) UNSIGNED NOT NULL,
  `idNodeDependent` INT(10) UNSIGNED NOT NULL,
  `idUDRelation` INT(10) UNSIGNED NOT NULL,
  `created_at` TIMESTAMP NULL DEFAULT NULL,
  `updated_at` TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (`idLexiconPatternEdge`),
  UNIQUE INDEX `idx_lexicon_pattern_edge_head_dep` (`idLexiconPattern` ASC, `idNodeHead` ASC, `idNodeDependent` ASC) VISIBLE,
  INDEX `lexicon_pattern_edge_idnodehead_foreign` (`idNodeHead` ASC) VISIBLE,
  INDEX `lexicon_pattern_edge_idnodedependent_foreign` (`idNodeDependent` ASC) VISIBLE,
  INDEX `lexicon_pattern_edge_idudrelation_foreign` (`idUDRelation` ASC) VISIBLE,
  INDEX `lexicon_pattern_edge_idlexiconpattern_idudrelation_index` (`idLexiconPattern` ASC, `idUDRelation` ASC) VISIBLE,
  CONSTRAINT `lexicon_pattern_edge_idlexiconpattern_foreign`
    FOREIGN KEY (`idLexiconPattern`)
    REFERENCES `webtool42_db`.`lexicon_pattern` (`idLexiconPattern`)
    ON DELETE CASCADE,
  CONSTRAINT `lexicon_pattern_edge_idnodedependent_foreign`
    FOREIGN KEY (`idNodeDependent`)
    REFERENCES `webtool42_db`.`lexicon_pattern_node` (`idLexiconPatternNode`)
    ON DELETE CASCADE,
  CONSTRAINT `lexicon_pattern_edge_idnodehead_foreign`
    FOREIGN KEY (`idNodeHead`)
    REFERENCES `webtool42_db`.`lexicon_pattern_node` (`idLexiconPatternNode`)
    ON DELETE CASCADE,
  CONSTRAINT `lexicon_pattern_edge_idudrelation_foreign`
    FOREIGN KEY (`idUDRelation`)
    REFERENCES `webtool42_db`.`udrelation` (`idUDRelation`)
    ON DELETE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_unicode_ci;
...
-- -----------------------------------------------------
-- Table `webtool42_db`.`daisy_node`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`daisy_node` (
  `idDaisyNode` INT NOT NULL AUTO_INCREMENT,
  `name` VARCHAR(255) NULL,
  `type` CHAR(2) NULL,
  `idFrame` INT(11) UNSIGNED NULL,
  `idFrameElement` INT(11) UNSIGNED NULL,
  `idLU` INT(11) UNSIGNED NULL,
  PRIMARY KEY (`idDaisyNode`),
  INDEX `fk_daisy_node_frame1_idx` (`idFrame` ASC) VISIBLE,
  INDEX `fk_daisy_node_frameelement1_idx` (`idFrameElement` ASC) VISIBLE,
  INDEX `fk_daisy_node_lu1_idx` (`idLU` ASC) VISIBLE,
  CONSTRAINT `fk_daisy_node_frame1`
    FOREIGN KEY (`idFrame`)
    REFERENCES `webtool42_db`.`frame` (`idFrame`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_daisy_node_frameelement1`
    FOREIGN KEY (`idFrameElement`)
    REFERENCES `webtool42_db`.`frameelement` (`idFrameElement`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_daisy_node_lu1`
    FOREIGN KEY (`idLU`)
    REFERENCES `webtool42_db`.`lu` (`idLU`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- -----------------------------------------------------
-- Table `webtool42_db`.`daisy_link`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`daisy_link` (
  `idDaisyLink` INT NOT NULL AUTO_INCREMENT,
  `value` FLOAT NULL,
  `type` CHAR(3) NULL,
  `idDaisyNodeSource` INT NOT NULL,
  `idDaisyNodeTarget` INT NOT NULL,
  PRIMARY KEY (`idDaisyLink`),
  INDEX `fk_daisy_link_daisy_node1_idx` (`idDaisyNodeSource` ASC) VISIBLE,
  INDEX `fk_daisy_link_daisy_node2_idx` (`idDaisyNodeTarget` ASC) VISIBLE,
  CONSTRAINT `fk_daisy_link_daisy_node1`
    FOREIGN KEY (`idDaisyNodeSource`)
    REFERENCES `webtool42_db`.`daisy_node` (`idDaisyNode`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_daisy_link_daisy_node2`
    FOREIGN KEY (`idDaisyNodeTarget`)
    REFERENCES `webtool42_db`.`daisy_node` (`idDaisyNode`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`view_annotation` (`idAnnotation` INT, `status` INT, `idEntity` INT, `entityType` INT, `idTextspan` INT, `idStaticObject` INT, `idDynamicObject` INT, `idDocument` INT, `idUserTask` INT, `idTask` INT, `name` INT, `idUser` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`view_lexicon` (`idWordForm` INT, `form` INT, `idEntityWF` INT, `idLanguage` INT, `idLexiconGroup` INT, `idLexiconExpression` INT, `position` INT, `breakBefore` INT, `head` INT, `idLemma` INT, `lemma` INT, `idEntityLM` INT, `idLanguageLM` INT, `idPOSLM` INT, `posLM` INT, `udPOSLM` INT, `idLU` INT, `lu` INT, `senseDescription` INT, `incorporatedFE` INT, `idFrame` INT, `idEntityLU` INT, `statusLU` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`view_annotation_static` (`idStaticObject` INT, `name` INT, `scene` INT, `idFlickr30kEntitiesChain` INT, `nobndbox` INT, `origin` INT, `idLayerType` INT, `layerOrder` INT, `nameLayerType` INT, `layerGroup` INT, `idLanguage` INT, `idImage` INT, `idDocument` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`view_lucandidate` (`frameName` INT, `idFrame` INT, `idLU` INT, `name` INT, `senseDescription` INT, `discussion` INT, `idDocumentSentence` INT, `idStaticObject` INT, `idDynamicObject` INT, `origin` INT, `idUser` INT, `incorporatedFE` INT, `createdAt` INT, `updatedAt` INT, `idLexicon` INT, `lemmaName` INT, `idPOS` INT, `idUDPOS` INT, `idLanguage` INT, `userName` INT, `email` INT);
...
-- -----------------------------------------------------
-- Placeholder table for view `webtool42_db`.`view_annotation_static_fe`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`view_annotation_static_fe` (`idAnnotation` INT, `idStaticObject` INT, `idFrameElement` INT, `idFrame` INT, `frameName` INT, `name` INT, `idEntity` INT, `idLanguage` INT, `bgColor` INT, `fgColor` INT);

-- -----------------------------------------------------
-- Placeholder table for view `webtool42_db`.`view_annotation_static_lu`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`view_annotation_static_lu` (`idAnnotation` INT, `idStaticObject` INT, `idLU` INT, `idEntity` INT, `name` INT, `frameName` INT, `idLanguage` INT);

-- -----------------------------------------------------
-- Placeholder table for view `webtool42_db`.`view_staticobject`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`view_staticobject` (`idStaticObject` INT, `name` INT, `scene` INT, `idFlickr30kEntitiesChain` INT, `nobndbox` INT, `origin` INT, `idLayerType` INT, `layerOrder` INT, `nameLayerType` INT, `layerGroup` INT, `idLanguage` INT, `idImage` INT, `idDocument` INT);

-- -----------------------------------------------------
-- Placeholder table for view `webtool42_db`.`view_lexicon_mwe`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `webtool42_db`.`view_lexicon_mwe` (`idLemma` INT);
...
CREATE FUNCTION lu_create(
    ...
    BEGIN
        ...
        DECLARE v_suggestedNewFrame VARCHAR(45);
        INSERT INTO entity (type) values ('LU');
        SET @idEntity = (SELECT last_insert_id());
        SET v_name = JSON_UNQUOTE(JSON_EXTRACT(par_lu, '$.name'));
        SET v_idFrame = NULLIF(JSON_UNQUOTE(JSON_EXTRACT(par_lu, '$.idFrame')), 'null');
        ...
        SET v_suggestedNewFrame = JSON_UNQUOTE(JSON_EXTRACT(par_lu, '$.suggestedNewFrame'));
        ...
        INSERT INTO lu(idFrame, name, senseDescription, importNum, active, incorporatedFE, idLexicon, idEntity,
			discussion,idDocumentSentence,idStaticObject,idDynamicObject,status,origin,createdAt, idUser, suggestedNewFrame) 
			values (v_idFrame, v_name, v_senseDescription, 0, 1, v_incorporatedFE, v_idLexicon, @idEntity,
            v_discussion,v_idDocumentSentence,v_idStaticObject,v_idDynamicObject,v_status,v_origin,NOW(), v_idUser, v_suggestedNewFrame);
...
BEGIN 
   ...
   INSERT INTO dynamicobject (name,startFrame,endFrame,startTime,endTime,origin,status,idLayerType)
	values (v_name,v_startFrame,v_endFrame,v_startTime,v_endTime,v_origin,'CREATED',v_idLayerType);
...
CREATE FUNCTION dynamicobject_delete(
    ...
    BEGIN
        ...
        UPDATE dynamicobject SET status = 'DELETED' WHERE idDynamicObject = par_idDynamicObject;
...
BEGIN 
   ...
   INSERT INTO staticobject (name,scene,nobndbox,idFlickr30kEntitiesChain, origin, idLayerType, status)
	values (v_name,v_scene,v_nobndbox,v_idFlickr30kEntitiesChain, v_origin, v_idLayerType, 'CREATED');
...
CREATE FUNCTION staticobject_delete(
    ...
    BEGIN
        ...
        -- DELETE FROM boundingbox where idBoundingBox in (select idBoundingBox from staticobject_boundingbox WHERE idStaticObject = par_idStaticObject);
        -- DELETE FROM staticobject_boundingbox WHERE idStaticObject = par_idStaticObject;
        -- DELETE FROM staticobject_textspan WHERE idStaticObject = par_idStaticObject;
        -- DELETE FROM annotationobjectrelation WHERE (idAnnotationObject1 = v_idAnnotationObject) oR (idAnnotationObject2 = v_idAnnotationObject);
		-- DELETE FROM staticobject WHERE idStaticObject = par_idStaticObject;	
        -- DELETE FROM annotationobject WHERE idAnnotationObject = v_idAnnotationObject;
        UPDATE staticobject SET status = 'DELETED' WHERE idStaticObject = par_idStaticObject;
...
CREATE FUNCTION annotationset_hard_delete(
    ...
    BEGIN
		...
		DELETE FROM ascomments WHERE idAnnotationSet = par_idAnnotationSet;
		DELETE FROM textspan WHERE idLayer IN (SELECT idLayer FROM layer WHERE idAnnotationSet = par_idAnnotationSet);	
		DELETE FROM textspan WHERE idAnnotationSet = par_idAnnotationSet;	
        DELETE FROM layer WHERE idAnnotationSet = par_idAnnotationSet;
		DELETE FROM annotationset WHERE idAnnotationSet = par_idAnnotationSet;
        SELECT login INTO @login FROM user WHERE idUser = par_idUser;
        INSERT INTO timeline (tlDateTime,author,operation,tableName,id,idUser) VALUES (NOW(),@login,'D','annotationset', par_idAnnotationSet,par_idUser);
...
select frame.idLanguage as idLanguageFrame, frame.idFrame, frame.idEntity as frameIdEntity, frame.name frameName,
    lu.idLU, lu.name, lu.senseDescription, lu.active, lu. importNum, lu.incorporatedFE, lu.idEntity, lu.idLexicon, lu.status, lu.origin,
    lemma.name as lemmaName, lemma.idPOS, lemma.idUDPOS, lemma.idLanguage, lemma.idLexicon as idLemma
from lu 
...
select a.idAnnotation, a.status, a.idEntity, e.type entityType, 
...
select wf.idLexicon as idWordForm, wf.form, wf.idEntity as idEntityWF, wf.idLanguage, wf.idLexiconGroup,
       ...
       lu.idLU, lu.name as lu, lu.senseDescription, lu.incorporatedFE, lu.idFrame, lu.idEntity as idEntityLU, lu.status as statusLU
    from lexicon wf
...
WHERE ((dob.origin = 1) or (dob.origin = 2)) and (dob.status <> 'DELETED')
...
SELECT
    sob.idStaticObject,sob.name,sob.scene,sob.idFlickr30kEntitiesChain,sob.nobndbox,sob.origin,
    lt.idLayerType, lt.layerOrder, lte.name as nameLayerType, lg.name as layerGroup, lte.idLanguage,
    iso.idImage,di.idDocument
...
WHERE (dob.origin = 5) and (dob.status <> 'DELETED')
GROUP BY dob.idDynamicObject, dob.name, dob.startFrame, dob.endFrame,
         dob.startTime, dob.endTime, dob.status, dob.origin,
         lt.idLayerType, lt.layerOrder, lte.name, lg.name, lte.idLanguage;
...
select frame.name as frameName, frame.idFrame,
    lu.idLU, lu.name, lu.senseDescription, lu.discussion,
...
-- -----------------------------------------------------
-- View `webtool42_db`.`view_annotation_static_fe`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `webtool42_db`.`view_annotation_static_fe`;
USE `webtool42_db`;
CREATE  OR REPLACE VIEW `view_annotation_static_fe` AS
SELECT a.idAnnotation, a.idStaticObject, 
	fe.idFrameElement, fe.idFrame, f.name as frameName, fee.name, fe.idEntity, fee.idLanguage,
	c.rgbBg as bgColor, c.rgbFg as fgColor
FROM annotation a
	JOIN staticobject sob on (a.idStaticObject = sob.idStaticObject)
    JOIN frameelement fe on (a.idEntity = fe.idEntity)
    JOIN color c ON (fe.idColor = c.idColor)
    JOIN frame fr ON (fe.idFrame = fr.idFrame)
    JOIN entry f ON (fr.idEntity = f.idEntity)
    JOIN entry fee ON (fe.idEntity = fee.idEntity)
WHERE (f.idLanguage = fee.idLanguage)
AND (a.status <> 'DELETED');

-- -----------------------------------------------------
-- View `webtool42_db`.`view_annotation_static_lu`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `webtool42_db`.`view_annotation_static_lu`;
USE `webtool42_db`;
CREATE  OR REPLACE VIEW `view_annotation_static_lu` AS
SELECT a.idAnnotation, a.idStaticObject, 
	lu.idLU, lu.idEntity, lu.name, f.name as frameName, f.idLanguage
FROM annotation a
	JOIN staticobject sob on (a.idStaticObject = sob.idStaticObject)
    JOIN lu on (a.idEntity = lu.idEntity)
    JOIN frame fr ON (lu.idFrame = fr.idFrame)
    JOIN entry f ON (fr.idEntity = f.idEntity)
WHERE (lu.status = 'CREATED')
AND (a.status <> 'DELETED');

-- -----------------------------------------------------
-- View `webtool42_db`.`view_staticobject`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `webtool42_db`.`view_staticobject`;
USE `webtool42_db`;
CREATE  OR REPLACE VIEW `view_staticobject` AS
select sob.idStaticObject,sob.name,sob.scene,sob.idFlickr30kEntitiesChain,sob.nobndbox,sob.origin,
	lt.idLayerType, lt.layerOrder, lte.name as nameLayerType, lg.name as layerGroup, lte.idLanguage,
	iso.idImage,di.idDocument
from staticobject sob
JOIN layertype lt ON (sob.idLayerType = lt.idLayerType)
...
-- -----------------------------------------------------
-- View `webtool42_db`.`view_lexicon_mwe`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `webtool42_db`.`view_lexicon_mwe`;
USE `webtool42_db`;
create or replace view view_lexicon_mwe as
select lemma.idLexicon as idLemma
from lexicon as lemma
join lexicon_expression le on (lemma.idLexicon = le.idLexicon)
join lexicon_group as lg on (lemma.idLexiconGroup = lg.idLexiconGroup)
join udpos on (lemma.idUDPOS = udpos.idUDPOS)
join language on (lemma.idLanguage = language.idLanguage)
where (lemma.idLexiconGroup = 2)
group by lemma.idLexicon
having count(distinct le.position) > 1;
...
USE `webtool42_db`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`fnbrasil`@`%` SQL SECURITY DEFINER VIEW `webtool42_db`.`view_wflexemelemma` AS select `wf`.`idWordForm` AS `idWordForm`,`wf`.`form` AS `form`,`wf`.`md5` AS `md5`,`lx`.`idLexeme` AS `idLexeme`,`lx`.`name` AS `lexeme`,`pos1`.`idPOS` AS `idPOSLexeme`,`pos1`.`POS` AS `POSLexeme`,`lx`.`idLanguage` AS `idLanguage`,`le`.`idLexemeEntry` AS `idLexemeEntry`,`le`.`lexemeOrder` AS `lexemeOrder`,`le`.`breakBefore` AS `breakBefore`,`le`.`headWord` AS `headWord`,`lm`.`idLemma` AS `idLemma`,`lm`.`name` AS `lemma`,`pos2`.`idPOS` AS `idPOSLemma`,`pos2`.`POS` AS `POSLemma`,`lang`.`language` AS `language` from ((((((`webtool42_db`.`wordform` `wf` join `webtool42_db`.`lexeme` `lx` on(`wf`.`idLexeme` = `lx`.`idLexeme`)) join `webtool42_db`.`pos` `pos1` on(`lx`.`idPOS` = `pos1`.`idPOS`)) join `webtool42_db`.`language` `lang` on(`lx`.`idLanguage` = `lang`.`idLanguage`)) left join `webtool42_db`.`lexemeentry` `le` on(`lx`.`idLexeme` = `le`.`idLexeme`)) left join `webtool42_db`.`lemma` `lm` on(`le`.`idLemma` = `lm`.`idLemma`)) left join `webtool42_db`.`pos` `pos2` on(`lm`.`idPOS` = `pos2`.`idPOS`)) where `lm`.`idLanguage` = `lx`.`idLanguage` or `lm`.`idLanguage` is null;
